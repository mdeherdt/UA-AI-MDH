...
def main():
    ap = argparse.ArgumentParser(description="Likelihood Weighting with multiple evidence")
    ...
    for i in range(args.runs):
        rng = random.Random(i)
        p = likelihood_weighting(bn, (args.qvar, args.qval), evidence, args.N, rng)
    ...
    print(f"\n[Likelihood Weighting] {args.runs} runs × {args.N} samples each")
